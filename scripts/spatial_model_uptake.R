######################################
### Spatial Model - uptake of LFTs ###
######################################

# Purpose: To analyse uptake of LFTs in Liverpool using a spatial modelling framework.
...
# Subset liverpool LA and lft tests, for time period of analysis
...
source("./scripts/tidy_analytical_dataset.R")

# Get descriptives
source("./scripts/calculate_descriptives.R")
table1 # Print
write.csv(table1, "./output/descriptives_table_uptake.csv") # Save
...
plot_m1 <- ggplot(model_data) + # Plot
  ...
  scale_fill_viridis(values = scales::rescale(c(0.3,0.9,1,1.1,6.9)), # Define values for colours
                       limits = c(0.3, 6.9)) +
...
fe_table # Print
write.csv(fe_table, "./output/fe_uptake_model_raw.csv") # Save

fe_table_rr <- cbind(fe_table[,c(9,1)], exp(fe_table[,2:7])) # Calculate relative risk
write.csv(fe_table_rr, "./output/fe_uptake_model_rr.csv") # Save
...
re_table # Print
write.csv(re_table, "./output/re_uptake_models.csv") # Save
...
write.csv(mlik_table, "./output/mlik_uptake_model.csv") # Save
...
ggsave(plot = map, filename = "./output/rr_maps_uptake_highres.tiff", dpi = 300)
ggsave(plot = map, filename = "./output/rr_maps_uptake_lowres.jpeg")
ggsave(plot = map, filename = "./output/rr_maps_uptake.svg")
...
fe_table2 # Print
write.csv(fe_table2, "./output/fe_uptake_sensitivity_raw.csv") # Save

fe_table2 <- exp(fe_table2) # Calculate relative risk
write.csv(fe_table2, "./output/fe_uptake_sensitivity_rr.csv") # Save
...
write.csv(mlik_table, "./output/mlik_uptake_sensitivity_model.csv") # Save
