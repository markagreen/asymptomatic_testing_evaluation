...
# Purpose: To analyse use a multi-level modelling framework to undertake individual-level analysis as sensntivity analyses.
...
lft_persons <- lft_tests[, list(positive = max(positive), age = max(age), imd_score = max(imd_score), prop_students = max(prop_students), ch_prop = max(ch_prop), walkDistAvg = max(walkDistAvg), num_test = max(num_test), t_date = max(t_date)), by = list(pid, sex, lsoa11, grp_label, eth_group_imp)]
...
# Model
model1 <- glmer(positive ~ 1 + age + sex + factor(eth_group_imp) + imd_score + prop_students + ch_prop + walkDistAvg + factor(grp_label) + (1|lsoa11), family="binomial", data = lft_persons)
summary(model1)
...
# Model
model2 <- glmer(multiple_lfts ~ 1 + age + sex + factor(eth_group_imp) + imd_score + prop_students + ch_prop + walkDistAvg + factor(grp_label) + (1|lsoa11), family="binomial", data = lft_persons)
summary(model2)
