...
lft_tests=testing[lft==1 & lad11cd=="E08000012" & positive==1]
...
# source("./scripts/test_descriptives.R") # Calculate denominators for above (by tests)
...
plot_m1 <- ggplot(model_data) + # Plot
  ...
  scale_fill_gradientn(colours = c("blue", "white", "red"), # Define colours to plot
                       values = scales::rescale(c(0.2,0.9,1,1.1,3.8)), # Define values for colours
                       limits = c(0.2, 3.8)) +
...
write.csv(fe_table, "./output/fe_positivity_model_rr.csv") # Save
...
fe_table2 # Print
write.csv(fe_table2, "./output/fe_positivity_sensitivity_raw.csv") # Save

fe_table2 <- exp(fe_table2) # Calculate relative risk
write.csv(fe_table2, "./output/fe_positivity_sensitivity_rr.csv") # Save
